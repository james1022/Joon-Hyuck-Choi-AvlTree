 ...
 *
 */
public class DuplAvlTree<T extends Comparable<? super T>> 
    ...
    /** The number of spaces for each level. 
        This is the specified number in the assignment.
    ...
    /** Contains the size of the Avl tree. */
    protected int size;
    ...
    public void insert(T x) {      
        if (this.contains(x)) {
            this.root = this.dupInsert(x, this.root);
        } else {
    ...
    /**
     * Internal insert method to be used when the item to insert already exists
     * in the tree, and just the number of copies needs to be increased.
     * @param x the duplicate item to be inserted.
     * @param curr the current node being visited.
     * @return the resultant tree, with the number of copies of the duplicate
     *         value incremented by 1.
     */
    private AvlNode<T> dupInsert(T x, AvlNode<T> curr) {
        
        int compareResult = x.compareTo(curr.element);
        
        if (compareResult < 0) { //go left
            curr.left = this.dupInsert(x, curr.left);
        } else if (compareResult > 0) { //go right
            curr.right = this.dupInsert(x, curr.right);
        } else if (compareResult == 0) { //increment number of copies
            curr.copies++;
        }
     ...
     * Deals with both duplicate case and non-duplicate case.
     * @param x the item to remove.
    ...
    private AvlNode<T> remove(T x, AvlNode<T> curr) {
        ...
        if (compResult < 0) { //go left
            ...
            if (curr.copies == 1) { //if there's only 1 of the node, do normal
                curr.element = this.findMin(curr.right).element;    
                curr.right = this.remove(curr.element, curr.right);
            } else if (curr.copies > 1) { //if there are duplicates, decrement
                curr.copies--;
            }
        } else { //only one child
            if (curr.copies == 1) { //same as AvlTree.java
                if (curr.left != null) {
                    curr = curr.left;
                } else {
                    curr = curr.right;
                }
            } else { //duplicates exist, so just decrement
                curr.copies--;
            }
    ...
    private void printTree(AvlNode<T> curr) {
        if (curr != null) {
            ...
            //print as many times as the element appears
            for (int i = 0; i < curr.copies; i++) {
    ...
    private void sidewaysPrintTree(AvlNode<T> curr) {
        ...
        if (curr != null) {
            ...
            //amount of indentation for each item
            for (int i = 0; i < numSpace; i++) {
                System.out.print(" ");
            }
           
            if (curr.copies == 1) { //normal operation
                System.out.println(curr.element);
            } else if (curr.copies > 1) { //print how many copies exist
                System.out.println(curr.element + "(" + curr.copies + ")");
            }
    ...
    private void reportValuesInRange(ArrayList<T> pList, 
        ...
        if (highComp > 0) { //go left
            this.reportValuesInRange(pList, low, high, curr.left);
        } else if (lowComp < 0) { //go right
            this.reportValuesInRange(pList, low, high, curr.right);
        } else if (lowComp > 0 && highComp < 0) { //left, add, right
            this.reportValuesInRange(pList, low, high, curr.left);
            this.addCopies(pList, curr.copies, curr); //add as many as necessary
            this.reportValuesInRange(pList, low, high, curr.right);
        } else if (lowComp == 0) { //add, right
            this.addCopies(pList, curr.copies, curr);
            this.reportValuesInRange(pList, low, high, curr.right);
        } else if (highComp == 0) { //add, left
            this.reportValuesInRange(pList, low, high, curr.left);
            this.addCopies(pList, curr.copies, curr);
        } //end all cases
    ...
    /**
     * Adds as many copies of an item as specified into an ArrayList.
     * This helper method was written to avoid the high cyclicality problem.
     * This part may as well have been written inside 
     * the private reportValuesInRange method, but was separated outside.
     * 
     * @param list the given list.
     * @param copies the number of copies to insert.
     * @param curr the current node being looked at.
     */
    private void addCopies(ArrayList<T> list, int copies, AvlNode<T> curr) {
    ...
    private static class AvlNode<T> {
        ...
        int height;
        /** Keeps track of how many instances of 
         * the same value are in the tree. */
        int copies;
    ...
    private class AvlIterator implements Iterator<T> {
         ...
         */
        public AvlIterator() {  
            this.list = new ArrayList<>(DuplAvlTree.this.size());
            this.current = 0;
            DuplAvlTree.this.postOrder(DuplAvlTree.this.root, this.list);
        } //end constructor
        ...
        public boolean hasNext() {
            return this.current < DuplAvlTree.this.size();
        } //end hasNext()
